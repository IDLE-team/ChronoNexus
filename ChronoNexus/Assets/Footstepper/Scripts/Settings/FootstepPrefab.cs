...
namespace GamingIsLove.Footsteps
{
	...
	public class FootstepPrefab
	{
		...
		[Space(10)]
		public List<FootstepIndexPrefab> indexPrefabs = new List<FootstepIndexPrefab>();
		...
		public virtual IEnumerator CreatePrefab(Transform origin, Transform foot, int footIndex, Vector3 hitPosition, Vector3 hitNormal)
		{
			bool found = false;
			for(int i=0; i<this.indexPrefabs.Count; i++)
			{
				if(this.indexPrefabs[i].footIndex == footIndex)
				{
					found = true;
					yield return this.indexPrefabs[i].CreatePrefab(origin, foot, hitPosition, hitNormal);
				}
			}
