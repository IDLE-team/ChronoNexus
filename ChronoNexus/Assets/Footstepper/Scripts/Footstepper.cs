...
namespace GamingIsLove.Footsteps
{
	...
	public class Footstepper : MonoBehaviour
	{
		...
		public bool topDown2DMode = false;
		...
		[Tooltip("Search for renderers on the hit game object, using the renderer's main texture/sprite/material to find a footstep effect.\n" +
			"If a 'Mesh Renderer' was found, tries to find a footstep effect based on the material at the hit position.\n" +
			"Requires a 'Footstep Manager' in the scene.")]
		...
		[Tooltip("Fired whenever this Footstepper plays a footstep with more detailed information: " +
			"The foot's 'Transform', the 'FootstepEffect', the 'Vector3' position hit by the raycast and the 'Vector3' normal of the hit position.")]
		...
		*/
		...
		public virtual bool AddOverrideSource(FootstepSource footstepSource)
		{
			if(!this.overrideSources.Contains(footstepSource))
			{
		...
		public virtual FootstepEffect GetOverrideEffect(RaycastResult result, Vector3 position)
		{
			...
			for(int i = this.overrideSources.Count - 1; i >= 0; i--)
			{
				...
				{
					effect = this.overrideSources[i].GetFootstepAt(result, position, this.effectTag);
					if(effect != null)
					{
						return effect;
					}
		...
		*/
		...
		public virtual bool IsEnabled
		{
			get
			{
				return FootstepperMode.Disabled != this.mode &&
					FootstepManager.IsAllowed(this);
			}
		}
		...
		*/
		...
		public virtual void PlayFootstep(Transform foot, int footIndex, float volume, FootstepType type, string customName)
		{
			if(this.timeout <= 0)
			{
				Vector3 hitPosition;
				...
				if(effect != null)
				{
					...
					if(this.IsPrefabEnabled)
					{
						...
						if(prefab != null &&
							prefab.prefab != null)
						{
							this.StartCoroutine(prefab.CreatePrefab(this.transform, foot, footIndex, hitPosition, hitNormal));
						}
					}
					...
					this.onFootstepDetailed.Invoke(foot, effect, hitPosition, hitNormal);
				}
			}
		}
		...
		public virtual void PlayFootstep(Transform foot, int footIndex, FootstepSource footstepSource, bool raycastPosition)
		{
			if(this.IsEnabled &&
				this.timeout <= 0)
			{
				...
				{
					effect = this.GetOverrideEffect(result, hitPosition);
				}
				if(effect == null)
				{
					effect = footstepSource.GetFootstepAt(result, hitPosition, this.effectTag);
				}
		...
		public virtual FootstepEffect FindFootstep(Transform foot, out Vector3 hitPosition, out Vector3 hitNormal)
		{
			...
			if(this.overrideSources.Count > 0)
			{
				FootstepEffect effect = this.GetOverrideEffect(result, foot.position);
				if(effect != null)
				{
			...
			if(result != null)
			{
				...
				{
					FootstepEffect effect = footstepSource.GetFootstepAt(result, foot.position, this.effectTag);
					if(effect != null)
					{
						return effect;
					}
				...
				if(FootstepManager.Instance != null)
				{
					...
					if(this.searchRenderers)
					{
						...
						if(renderer != null)
						{
							...
							{
								Material material = MeshFootstepSource.GetMaterial(
									MeshFootstepSource.GetMesh(result.transform.gameObject), 
									renderer, result.triangleIndex);
								if(material != null)
								{
									FootstepEffect effect = FootstepManager.Instance.GetFootstepFor(material, result.textureCoord, this.effectTag);
									if(effect != null)
									{
										return effect;
									}
							...
							{
								FootstepEffect effect = FootstepManager.Instance.GetFootstepFor(
									renderer.material.mainTexture, result.textureCoord, this.effectTag);
								if(effect != null)
								{
									return effect;
								}
								...
								{
									effect = FootstepManager.Instance.GetFootstepFor(
										renderer.sharedMaterial, result.textureCoord, this.effectTag);
									if(effect != null)
									{
										return effect;
									}
		...
		*/
		...
		public virtual void FootstepIndex(int index)
		{
					...
					this.speed.x >= this.minSpeed))
			{
				...
				FootstepType type = FootstepType.Run;
				this.GetSpeedType(ref volume, ref type);
				this.PlayFootstep(this.GetFoot(index), index, volume, type, "");
			}
		}
		...
		public virtual void FootstepWalkIndex(int index)
		{
			...
			{
				this.PlayFootstep(this.GetFoot(index), index,
